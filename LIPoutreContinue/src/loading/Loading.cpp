...
Loading::Loading(std::vector<std::vector<std::vector<double>>> CURVES, std::vector<double> POSITION,
	...
	Configuration(), SpanNodePositions(SpanNodePositions), CURVES(CURVES), POSITION(POSITION)
{
	spans             = spans_in;
	...
	for (size_t span = 0; span < CopieSpan.size(); span++) {
		...
		for (size_t section = 0; section < SpanNodePositions[span].size(); section++) {
			...
			for (auto& k : Point_LOAD) {
				...
				// POSITION[max_position] = first_wall optimal → +length[0] donne la vraie position
				RR["Position"] = POSITION[lo.max_position] + k.Length[0];
				TT[k.name] = RR;
			}
	...
	for (size_t span = 0; span < CopieSpan.size(); span++) {
		...
		for (size_t section = 0; section < SpanNodePositions[span].size(); section++) {
			...
			for (auto& k : Rectangulare_LOAD) {
				std::map<std::string, double> RR;
				Position1D lo = PluralRectangularLoad(k.Intensity, k.Length, span, section);
				val += lo.value;
				RR["alpha"] = static_cast<double>(lo.max_position);
				RR["value"] = lo.value;
	...
	first = {};
	ValuesPerSpan = {};
	std::vector<double> PositionsPerSpan;       // position optimale du convoi par travée
	...
	for (size_t span = 0; span < CopieSpan.size(); span++) {
		...
		for (size_t section = 0; section < SpanNodePositions[span].size(); section++) {
			...
			PositionsPerSection.push_back(LO.position);  // POSITION[IJ] pour cette section
		...
		ValuesPerSpan.push_back(maximum_section);
		PositionsPerSpan.push_back(PositionsPerSection[critical_sec]); // position exacte pour cette travée
	...
	Combined_load.position = PositionsPerSpan[Combined_load.span]; // position exacte de la travée critique
...
double Loading::OnePointLoad(double intensity, size_t span, size_t section, size_t alpha)
{
	if (alpha >= CURVES[span][section].size()) {
		std::cout << "Warning: Alpha value exceeds the maximum defined in CURVES for span "
			<< span << " and section " << section << ". Returning 0." << std::endl;
		return 0;
	}
	return CURVES[span][section][alpha] * intensity;
}

Position1D Loading::PluralPointLoad(const std::vector<double>& intensity, const std::vector<double>& length, size_t span, size_t section)
{
	...
	for (auto& first_wall : POSITION) {
		...
		for (auto& I : intensity) {
			size_t alpha = MetersToPosition(length[compteur] + first_wall);
...
double Loading::OneRectangularLoad(double intensity, size_t span, size_t section, size_t begin, size_t end)
{
...
//    length[0]          = offset de départ du convoi (distance avant le 1er segment)
//    length[1..N]       = longueurs successives des segments
//
...
// =============================================================================
Position1D Loading::PluralRectangularLoad(const std::vector<double>& intensity, const std::vector<double>& length, size_t span, size_t section)
{
	// Calcul des positions successives relatives au début du convoi
	// LengthSuccessive[k] = distance depuis le bord gauche du convoi jusqu'au début du segment k
	// LengthSuccessive[k+1] = distance jusqu'à la fin du segment k
	...
	for (const auto& first_wall : POSITION) {
		...
		for (size_t k = 0; k < intensity.size(); ++k) {
			...
			if (begin >= POSITION.size()) begin = 0;
			if (end >= POSITION.size()) end = POSITION.size() - 1;
...
Loading::CriticalSectionResult Loading::computeCriticalSection(size_t span)
{
	...
	// ── Charge ponctuelle ────────────────────────────────────────────────────
	{
		...
		std::map<std::string, std::map<std::string, double>> TT;
		for (auto& k : Point_LOAD) {
			auto lo = PluralPointLoad(k.Intensity, k.Length, span, result.point.section);
			TT[k.name] = { {"alpha",(double)lo.max_position}, {"value",lo.value},
			               {"Position", POSITION[lo.max_position] + k.Length[0]} };
		}
	...
	// ── Charge répartie ──────────────────────────────────────────────────────
	{
		...
		std::map<std::string, std::map<std::string, double>> TT;
		for (auto& k : Rectangulare_LOAD) {
			auto lo = PluralRectangularLoad(k.Intensity, k.Length, span, result.rect.section);
			TT[k.name] = { {"alpha",(double)lo.max_position}, {"value",lo.value},
			               {"Position", POSITION[lo.max_position] + k.Length[0]} };
		}
		result.rect.load = TT;
...
// =============================================================================
//  CombinedLoad — Fix bug 3 (position UDL) + cohérence avec les corrections ci-dessus
// =============================================================================
		...
		for (auto& j : Point_LOAD) {
			...
			for (auto& I : j.Intensity) {
				...
				double contribution = (alpha < POSITION.size())
					...
					: 0.0;
				val += contribution;
				third[std::to_string(static_cast<int>(I)) + " " + std::to_string(compteur + 1)] = contribution;
				++compteur;
			}
		...
		for (auto& jk : Rectangulare_LOAD) {
			...
			for (size_t k = 0; k < jk.Intensity.size(); ++k) {
				...
				if (x_end_m > x_begin_m) {
					...
					if (end >= POSITION.size()) end = POSITION.size() - 1;
					if (end > begin)
						contribution = OneRectangularLoad(jk.Intensity[k], span, section, begin, end);
				}
