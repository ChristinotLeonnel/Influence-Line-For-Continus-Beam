...
├── Input.h / Input.cpp     # Configuration file parser
├── Isostatique.h/.cpp      # Simply-supported (isostatic) beam influence lines
├── Hyperstatique.h/.cpp    # Continuous beam solver (three-moment method)
├── Loading.h / Loading.cpp # Load positioning and envelope computation
├── Output.h / Output.cpp   # Full pipeline: compute → find max → export JSON
├── UpdatePositions.h/.cpp  # Rewrites the input config with optimal load positions
├── JsonStreamWriter.h      # Streaming JSON writer (low memory)
├── SpanResult.h            # Per-span result struct
├── ProjectPaths.h          # Centralised output directory registry
├── Utils.h                 # Shared types, math helpers, JSON export utilities
└── json.hpp / json_fwd.hpp # nlohmann/json v3.11.3 (bundled)
```
...
```
cl /std:c++20 /O2 /EHsc Aplication.cpp Input.cpp Isostatique.cpp Hyperstatique.cpp Loading.cpp Output.cpp UpdatePositions.cpp /I. /Fe:StructuralAnalysis.exe
```

No external libraries are needed — `nlohmann/json` is bundled as a single header.
...
|---|---|---|
| [nlohmann/json](https://github.com/nlohmann/json) | 3.11.3 | ✅ (`json.hpp`) |
